...
from openbox.utils.multi_objective import (
...
class SAEAAdvisor(ModularEAAdvisor):
    ...
    def __init__(
        ...
        required_evaluation_count = (
            required_evaluation_count or self.ea.required_evaluation_count
        )
        ...
        # Default Acq
        acq = acq or (
            ("ehvic" if self.num_constraints > 0 else "ehvi")
            if self.num_objectives > 1 and ref_point
            else (
                ("mesmoc2" if self.num_constraints > 0 else "mesmo")
        ...
        self.constraint_surrogates: List[AbstractModel] = [
            build_surrogate(
                constraint_surrogate, config_space, self.rng or random, None
            )
        ...
        if acq in {"ehvi", "ehvic"} and ref_point:
            ...
            self.acq: AbstractAcquisitionFunction = build_acq_func(
                acq,
                self.objective_surrogates if mo_acq else self.objective_surrogates[0],
    ...
    def _sel(self, parent: List[Individual], sub: List[Individual]) -> List[Individual]:
        ...
        if self.num_objectives == 1:
            ...
            self.acq.update(
                model=self.objective_surrogates[0],
            ...
            if self.acq_type == "parego":
                ...
                self.acq.update(
                    model=self.objective_surrogates,
                    constraint_models=self.constraint_surrogates,
                    eta=get_chebyshev_scalarization(weights, Y)(
                        np.atleast_2d(mo_incumbent_values)
                    ),
                    num_data=num_config_evaluated,
                )
